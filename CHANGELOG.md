...
- Updated the neutral scapula position and scapula sliding. The default neutral scapula 
  position (medial) rotation have been updated to a more realistic posture.
  Additionally, the node at scapula ts on which scapula slides on the thorax have 
  been moved to provide a more realistic clearance between the scapula and the 
  ellipsoid sliding surface representing thorax. Thanks to Johanna Menze from the 
  Univesity of Bern from updating the model.
